...
mlb = mlb()
X, y= lsf('rcv1_regions/rcv1_regions.train',multilabel=True);
X = X[:10000]
y = y[:10000]
y = mlb.fit_transform(y)
y_new = []
idx = np.sum(y,axis=0)>100
for i in y:
    y_new.append(i[idx])
y = np.asarray(y_new)
...
#print('Class ratios :')
#print('Training set :',np.sum(y_train,axis=0),'Total',np.sum(y_train))
#print('Test set     :',np.sum(y_test,axis=0),'Total',np.sum(y_test))
#print('Pool set     :',np.sum(y_pool,axis=0),'Total',np.sum(y_pool))
...
#        print(i,err_gap[t][-1])
#
#        pick = gap_based_method(X_pool,clf,t=t+4,num=num)
#        idx = np.ones(X_pool.shape[0],dtype=bool)
#        idx[pick] = False
#        X_train = np.insert(X_train,(0),X_pool[pick],axis=0)
#        y_train = np.insert(y_train,(0),y_pool[pick],axis=0)
