...
for i in range(1,till):
    ...
    print(i,err_rand[-1],X_test.shape[0],X_train.shape[0])

    pick = np.random.randint(0,X_test.shape[0])
    idx = np.ones(X_test.shape[0],dtype=bool)
    idx[pick] = False
    X_train = np.insert(X_train,(0),X_test[pick],axis=0)
    y_train = np.insert(y_train,(0),y_test[pick],axis=0)
    X_test = X_test[idx]
    y_test = y_test[idx]
...
for i in range(1,till):
    ...
    print(i,err_closest[-1], X_test.shape[0])

    pick = closest_to_plane(X_test,clf)
    idx = np.ones(X_test.shape[0],dtype=bool)
    idx[pick] = False
    X_train = np.insert(X_train,(0),X_test[pick],axis=0)
    y_train = np.insert(y_train,(0),y_test[pick],axis=0)
...
for i in range(1,till):
    ...
    print(i,err_gap[-1], X_test.shape[0])

    pick = our_method(X_test,clf,t=1)
    idx = np.ones(X_test.shape[0],dtype=bool)
    idx[pick] = False
    X_train = np.insert(X_train,(0),X_test[pick],axis=0)
    y_train = np.insert(y_train,(0),y_test[pick],axis=0)
