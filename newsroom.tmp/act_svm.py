...
#print(newsgroups_train.data[0].shape)
...
#to check improvement provided by new data
randomErr=np.diff(np.asarray(randomErr));
closestErr=np.diff(np.asarray(closestErr));
